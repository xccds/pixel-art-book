...
本书的定位已经替你做了工具选择：**像素向 + 独立开发者 + 程序员读者 = Aseprite + Godot**。所以这里不展开长篇论证——1.1 节把"为什么这一对就是对的"压成三十行，剩下的篇幅全给 Aseprite 实际操作（1.2-1.7）。
...
选工具不是美术问题——是你和产出效率之间最直接的工程决策。一个判断框架：任何工具都用三个指标打分 [^1]：
...
本书聚焦像素，你的工具栈就是这一套——手绘和 3D 工具链会在继续04 中简介，那是像素之后的能力扩展方向。像素栈：
...
6. **《工具选择的底层逻辑》** — 三原则（入门成本/产出效率/天花板）的完整版决策框架。**为什么推荐：** 如果你将来要做非像素项目（继续04），三大工具栈对比与免费→付费四信号是完整的选工具框架。

---

### 1.12 本章引注

[^1] 三原则（入门成本/产出效率/天花板）、不可能三角、编程语言运行时类比、三大工具栈对比、Respawn 零成本工具栈验证、免费→付费四信号。本章 1.1 是它的精简重定向版（删去 2D 手绘栈与 3D 低模栈，仅保留像素栈）。
