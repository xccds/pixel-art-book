<!--
章名: 制作01 你的工具：Aseprite 与工具逻辑
来源: B ch01 + A ch15精简
文风: A声改写
合并要点:
  - B: Aseprite安装/界面/工具/图层/选区/镜像/色彩 为主体
  - A ch15三原则精简为'为什么Aseprite+Godot'论证
  - 全面改写为A声
  - 为每个Aseprite工具补类比(铅笔=单像素setter/墨线=直线算法/填色=flood fill BFS)
SVG: 新生成Aseprite界面简化示意 + 沿用A ch15三原则图精简
练习: —
-->
...
原计划这里有一整章"选兵器"（书 A 第 15 章《工具选择的底层逻辑》）——讲三原则、三大工具栈对比、免费→付费拐点。但本书的定位已经替你做了这个选择：**像素向 + 独立开发者 + 程序员读者 = Aseprite + Godot**。所以"选兵器"不是删掉——是**坍缩**成一段论证（1.1），把第 15 章几十页的决策框架压成"为什么这一对就是对的"的三十行，然后把节省下来的篇幅全给 Aseprite 的实际操作（1.2-1.7）。
...
### 1.1 选兵器为什么坍缩：Aseprite + Godot 的工程论证
...
> **如果只记住一句话：** 本书替你坍缩了"选兵器"——Aseprite + Godot 是像素独立开发者在三原则下"最不慢"的解。你现在该做的不是继续研究工具，是打开 Aseprite 开始画。
...
第四部开篇把"选兵器"坍缩成一段论证：入门成本/产出效率/天花板三原则的不可能三角下，Aseprite + Godot 是像素独立开发者"最不慢"的解——Aseprite 三项全占、Godot 默认 Nearest Neighbor 管线最短。剩下的篇幅全给了 Aseprite 操作：六个界面区域、七个核心工具——每个都映射到一个你熟悉的编程概念（单像素 setter、flood fill BFS、Bresenham 直线、z-indexed 画布栈、mask/SQL WHERE、几何变换矩阵、索引色调色板）。
